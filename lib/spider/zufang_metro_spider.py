...
from lib.zone.station import get_metro_stations
...
class ZuFangMetroSpider(BaseSpider):
    def collect_area_zufang_data(self, city_name, metro_info, fmt="csv"):
        """
        ...
        :return: None
        """
        district_name = area_dict.get(metro_info['name'], "")
        csv_file = self.today_path + \
            "/{0}_{1}.csv".format('line' + district_name, metro_info['name'])
        with open(csv_file, "w") as f:
            # 开始获得需要的板块数据
            zufangs = self.get_area_zufang_info(city_name, metro_info)
            # 锁定
        ...
        print("Finish crawl area: " + metro_info['name'] +
              ", save data to : " + csv_file)

    @staticmethod
    def get_area_zufang_info(city_name, metro_info):
        matches = None
        """
        通过爬取页面获取城市指定版块的租房信息
        :param city_name: 城市
        ...
        total_page = 1
        district_name = area_dict.get(metro_info['name'], "")
        chinese_district = district_name + '号线'
        chinese_area = metro_info['name']
        zufang_list = list()
        page = 'http://{0}.{1}.com/ditiezufang/{2}/'.format(
            city_name, SPIDER_NAME, metro_info['path'])
        print(page)
        ...
        try:
            ...
            print('total page for {0} {1}'.format(chinese_district, chinese_area), total_page)
        except Exception as e:
            print("\tWarning: only find one page for line {0}".format(metro_info['name']))
            print(e)

        # 从第一页开始,一直遍历到最后一页
        headers = create_headers()
        for num in range(1, total_page + 1):
            page = 'http://{0}.{1}.com/ditiezufang/{2}/pg{3}/'.format(
                city_name, SPIDER_NAME, metro_info['path'], num)
            ...
            for house_elem in house_elements:
                ...
                try:
                    ...
                    zufang = ZuFang(
                        getUsefulInfo(chinese_district, district_name + '号线'),
                        getUsefulInfo(chinese_area, metro_info['name']),
                        xiaoqu, layout,
                        size,
                        price,
                        brand
